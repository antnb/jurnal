---
layout: full_article
title: "Full Article Of APLIKASI METODE ROTATED GUMBEL COPULA UNTUK MENGESTIMASI VALUE AT RISK PADA INDEKS SAHAM PASAR ASIA"
author: "IKHSAN AKBAR, KOMANG DHARMAWAN, NI MADE ASIH"
description: "Full Article Value at Risk is a tool used to calculate the value of risk in investing The purpose of this study was to estimate VaR in the portfolio using the rotated Copula Gumbel "
categories: mtk
canonical_url: https://jurnal.harianregional.com/mtk/full-52586 
citation_abstract_html_url: "https://jurnal.harianregional.com/mtk/id-52586"  
citation_pdf_url: "https://jurnal.harianregional.com/mtk/full-52586"  
comments: true
tags:

---

{% include adsense2.html %}
Value at Risk is a tool used to calculate the value of risk in investing The purpose of this study was to estimate VaR in the portfolio using the rotated Copula Gumbel approach which originated from the Archimedean copula family Copula can provide an overview of the relationship between random VaRiables on a quantil scale which is very useful in explaining the interrelationships in extreme events This VaR calculation is used in portfolios from the Indonesian st

{% include adsense1.html %}

<div style="position: relative; width: 100%; max-width: 1000px;">
    <!-- PDF viewer container -->
    <div style="position: relative; padding-bottom: 100%; overflow: hidden;">
        <!-- PDF viewer -->
        {% include inarticle.html %}
        <object data="https://jurnal.harianregional.com/pdf/mtk/52586.pdf" type="application/pdf" width="100%" height="100%" style="position: absolute; top: 0; left: 0;">
            <!-- Fallback content for browsers that cannot display PDFs -->
            <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background-color: #f8f9fa; display: flex; justify-content: center; align-items: center;">
                        <div style="position: absolute; top: 10px; right: 10px; z-index: 9999;">
                {% include adsense2.html %}
            </div>
                <p>Sorry, your browser does not support embedded PDFs. <a href="https://jurnal.harianregional.com/pdf/mtk/52586.pdf" target="_blank">Click here to view it.</a></p>
            </div>
            <!-- AdSense ad code -->
            <div style="position: absolute; top: 10px; right: 10px; z-index: 9999;">
              {% include inarticle.html %}
            </div>
        </object>
    </div>

