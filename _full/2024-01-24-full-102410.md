---
layout: full_article
title: "Full Article Of Sistem Inspeksi Visual Penempatan Label Produk Lip Cream Line Menggunakan Metode Deep Learning"
author: "Moch Deny Triatmaja, Lie Jasa"
description: "Full Article SBL Single Bottom Labeller machine which was still not precise for placing the bottom label batches of lip cream products Postal operators must then check one by one "
categories: jte
canonical_url: https://jurnal.harianregional.com/jte/full-102410 
citation_abstract_html_url: "https://jurnal.harianregional.com/jte/id-102410"  
citation_pdf_url: "https://jurnal.harianregional.com/jte/full-102410"  
comments: true
tags:

---

{% include adsense2.html %}
SBL Single Bottom Labeller machine which was still not precise for placing the bottom label batches of lip cream products Postal operators must then check one by one to ensure product quality control so that there are bottlenecks Therefore this research uses the YOLOv5 deep learning method to solve this problem The YOLOv5 algorithm uses the idea of regression making it easier to learn generalizations target characteristics and solve speed problems The YOLO

{% include adsense1.html %}

<div style="position: relative; width: 100%; max-width: 1000px;">
    <!-- PDF viewer container -->
    <div style="position: relative; padding-bottom: 100%; overflow: hidden;">
        <!-- PDF viewer -->
        {% include inarticle.html %}
        <object data="https://jurnal.harianregional.com/pdf/jte/102410.pdf" type="application/pdf" width="100%" height="100%" style="position: absolute; top: 0; left: 0;">
            <!-- Fallback content for browsers that cannot display jte_pdfs -->
            <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background-color: #f8f9fa; display: flex; justify-content: center; align-items: center;">
                        <div style="position: absolute; top: 10px; right: 10px; z-index: 9999;">
                {% include adsense2.html %}
            </div>
                <p>Sorry, your browser does not support embedded jte_pdfs. <a href="https://jurnal.harianregional.com/pdf/jte/102410.pdf" target="_blank">Click here to view it.</a></p>
            </div>
            <!-- AdSense ad code -->
            <div style="position: absolute; top: 10px; right: 10px; z-index: 9999;">
                ngadsense
            </div>
        </object>
    </div>

