...
def generate_full_article_markdown(title, authors, abstract, canonical_url, keywords, numeric_part):
    ...
    canonical_url = f"https://jurnal.harianregional.com/kerthadesa/full-{numeric_part}"

    # Update the citation abstract HTML URL dynamically based on numeric_part
    citation_abstract_html_url = f"https://jurnal.harianregional.com/kerthadesa/id-{numeric_part}"
...
def download_and_rename_pdf(url, numeric_part):
    try:
        ...
        file_name = f"kerthadesa_pdfs/{numeric_part}.pdf"
...
def process_url(url):
    try:
        ...
        citation_output_text = re.sub(r'https://ojs\.unud\.ac\.id/index\.php/kerthadesa/article/view/(\d+)', r'https://jurnal.harianregional.com/kerthadesa/id-\1', citation_output_text)
        # Extract the numeric part from the URL
        numeric_part = re.search(r'\d+$', url).group()

        # Construct canonical URL
        canonical_url = f"https://jurnal.harianregional.com/kerthadesa/id-{numeric_part}"
        buat_url = f"https://jurnal.harianregional.com/kerthadesa/full-{numeric_part}"
        # Update the citation abstract HTML URL
        citation_abstract_html_url = f"https://jurnal.harianregional.com/kerthadesa/id-{numeric_part}"
        citation_pdf_url = f"https://jurnal.harianregional.com/kerthadesa/full-{numeric_part}"
