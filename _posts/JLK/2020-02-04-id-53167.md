---
layout: post
title: "Design Of Hybrid Cryptography With Vigenere Cipher And RSA Algorithm On IOT Data Security"
author: "Maria Okta Safira, I Komang Ari Mogi"
description: "In this paper two methods are used namely the vigenere cipher method and the RSA method The vigenere cipher method is an example of a symmetric algorithm while RSA is"
categories: jlk
canonical_url: https://jurnal.harianregional.com/jlk/id-53167
comments: true
citation_abstract_html_url: "https://jurnal.harianregional.com/jlk/id-53167"
citation_pdf_url: "https://jurnal.harianregional.com/jlk/full-53167"
tags:

---

## Authors:
Maria Okta Safira, I Komang Ari Mogi

## Abstract:
"In this paper two methods are used, namely the vigenere cipher method and the RSA method. The vigenere cipher method is an example of a symmetric algorithm, while RSA is an example of an asymmetric algorithm. The combination of these two methods is called hybrid cryptography which has the advantage in terms of speed during the encryption process. Each process, which is encryption and decryption, is carried out twice, so that security can be ensured. In the process of forming the key used the RSA method. In the encryption process using public keys that have been generated before when the key is formed. This public key is used in sending data to the recipient of a secret message where this key is used for the data encryption process. The Secret key is kept and will be used during the decryption process. There is a system architecture that describes how clients and servers communicate with each other over the internet using the TCP protocol where the client here is an IoT device and the server is a server."

### Keywords
*Keyword Not Available*

### Downloads:
Download data is not yet available.

{% include adsense.html %}
## References
References Not Available

### PDF:

{% include adsense1.html %}

<https://jurnal.harianregional.com/jlk/full-53167>

{% include adsense2.html %}

### Published
2020-02-04

### How To Cite
SAFIRA, Maria Okta; ARI MOGI, I Komang.  Design Of Hybrid Cryptography With Vigenere Cipher And RSA Algorithm On IOT Data Security.JELIKU (Jurnal Elektronik Ilmu Komputer Udayana), [S.l.], v. 8, n. 4, p. 475-485, feb. 2020. ISSN 2654-5101. Available at: <https://ojs.unud.ac.id/index.php/JLK/article/view/53167>. Date accessed: {{ site.time | date: "%d %b. %Y" }}. doi:https://doi.org/10.24843/JLK.2020.v08.i04.p14.

## Citation Format
ABNT, APA, BibTeX, CBE, EndNote - EndNote format (Macintosh & Windows), MLA, ProCite - RIS format (Macintosh & Windows), RefWorks, Reference Manager - RIS format (Windows only), Turabian

### Issue
Vol 8 No 4 (2020): JELIKU Volume 8 No 4, Mei 2020

### Section 
**Articles**

### Copyright 
{% include inarticle.html %}
<a href="http://creativecommons.org/licenses/by/4.0/" rel="license"><img src="https://i.creativecommons.org/l/by/4.0/88x31.png" alt="Creative Commons License" /></a>
This work is licensed under a <a href="http://creativecommons.org/licenses/by/4.0/" rel="nofollow">Creative Commons Attribution 4.0 International License</a>

{% include multiplex.html %}
