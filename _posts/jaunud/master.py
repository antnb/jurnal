...
folders = ['posts', 'full_jaunud', 'jaunud_pdfs']
...
def generate_full_article_markdown(title, authors, abstract, canonical_url, keywords, numeric_part):
    ...
    canonical_url = f"https://jurnal.harianregional.com/jaunud/full-{numeric_part}"

    # Update the citation abstract HTML URL dynamically based on numeric_part
    citation_abstract_html_url = f"https://jurnal.harianregional.com/jaunud/id-{numeric_part}"
...
<div style="position: relative; width: 100%; max-width: 1000px;">
    ...
    <div style="position: relative; padding-bottom: 100%; overflow: hidden;">
        ...
        <object data="https://jurnal.harianregional.com/pdf/jaunud/{numeric_part}.pdf" type="application/pdf" width="100%" height="100%" style="position: absolute; top: 0; left: 0;">
            ...
            <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background-color: #f8f9fa; display: flex; justify-content: center; align-items: center;">
                ...
                <p>Sorry, your browser does not support embedded jaunud_pdfs. <a href="https://jurnal.harianregional.com/pdf/jaunud/{numeric_part}.pdf" target="_blank">Click here to view it.</a></p>
            </div>
            <!-- AdSense ad code -->
            <div style="position: absolute; top: 10px; right: 10px; z-index: 9999;">
...
def download_and_rename_pdf(url, numeric_part):
    try:
        ...
        file_name = f"jaunud_pdfs/{numeric_part}.pdf"
...
def process_url(url):
    try:
        ...
        citation_output_text = re.sub(r'https://ojs\.unud\.ac\.id/index\.php/jaunud/article/view/(\d+)', r'https://jurnal.harianregional.com/jaunud/id-\1', citation_output_text)
        # Extract the numeric part from the URL
        numeric_part = re.search(r'\d+$', url).group()

        # Construct canonical URL
        canonical_url = f"https://jurnal.harianregional.com/jaunud/id-{numeric_part}"
        buat_url = f"https://jurnal.harianregional.com/jaunud/full-{numeric_part}"
        # Update the citation abstract HTML URL
        citation_abstract_html_url = f"https://jurnal.harianregional.com/jaunud/id-{numeric_part}"
        citation_pdf_url = f"https://jurnal.harianregional.com/jaunud/full-{numeric_part}"
...
'''
        ...
        full_article_file_name = f"full_jaunud/{publication_date}-full-{numeric_part}.md" if publication_date else f"full_articles/unknown-full-{numeric_part}.md"
